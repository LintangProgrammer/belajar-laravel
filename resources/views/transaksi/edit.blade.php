...
@section('content')
    <div class="container">
        <h3 class="mb-4">Edit Transaksi #{{ $transaki->kode_transaksi }}</h3>
        ...
        <form action="{{ route('transaksi.update', $transaksi->id) }}" method="POST">
            ...
            <div class="mb-3">
                ...
                <select name="id_pelanggan" id="id_pelanggan" class="form-select" required>
                    ...
                    @foreach ($pelanggan as $p)
                        <option value="{{ $p->id }}" {{ $transaki->id_pelanggan == $p->id ? 'selected' : '' }}>
                            {{ $p->nama }}
                        </option>
                    @endforeach
                </select>
            ...
            <div id="produk-wrapper">
                @foreach ($transaki->produks as $prodTrans)
            ...
            <div class="text-end mb-4">
                <h5>Total Harga: <span id="totalHarga">Rp{{ number_format($transaki->total_harga, 0, ',', '.') }}</span>
                </h5>
            </div>
